...
internet of things, and computing to improve healthcare. These systems are large in scale, operate in unpredictable environments and have moving parts.  This disrupts classical siloed system design, where systems are broken down into components from different domains, each with their own tools and design methodology. For instance, system implementations and algorithms are often tuned to the environment or technology. 
...
%\paragraph{Mapping and semantic representation of spaces}

% \paragraph{Quantifying the information, and semantics}
% The eventual goal of these systems is to provide useful information upon which some action can be taken (by the system or by a human). Current paradigm in information processing is to process all the available sensor information to make an inference. While convenient, this approach is not the best strategy if power and communication bandwidth is a constraint. In context of localization for instance, the user may wish to know which room they are in, or how far the nearest coffee machine is, or want to know the relation of a certain physical object to another. I am interested in 

% This type of semantics 

% They don't require 
% if the user only requires  required information is only at room-levelrather than computing a location estimate and then inferef

% One of the questions I am interested in is - how useful is all this information really? For applications where the functionality is well defined, it is easy to answer this question. For emerging applications where we cannot yet anticipate the possible use cases, this is a hard question. For instance, consider the role of 
...
%These rely on accurate location. %We can create reconfigurable arrays with mobile communication agents that can be accurately localized. 
%Location plays a key role in emerging communication technologies such as mmWave as part of 5G. 

%One of the challenges that would be faced by reconfigurable mobile arrays is estimating the geometry of the array based on the environment. 
 %the r
...
% is to solve the problems of deployment (where to place? how to automatically map? ) and system performance (accuracy, instant)
% jointly. For this, I created new models, 
% First, 
%not be constrained by what technologies are available today. Time-of-flight is 
